...
bool aduio_RegWrite(alt_u8 reg_index, alt_u16 data16){
    ...
    bSuccess = I2C_AUDIO_Write(control, data);
    if (!bSuccess) 
...
bool audio_Play1K(void){
    ...
    static short szData[48];
    short sample;
    bool bTimeout, bSuccess = FALSE;
    alt_u32 time_start;
    int i;
    
    if (!gbAudioInit){
        AUDIO_DEBUG(("[AUDIO] play1k, not init\r\n"));
        return FALSE;
    }        
    ...
    // send data to dac-fifo    
    i = 0;
    time_start = alt_nticks();  
    bTimeout = FALSE;
    while(i<256 && !bTimeout){
        if (!AUDIO_DACFIFO_ISFULL()){
            sample = szData[index];
            AUDIO_DACFIFO_WRITE(sample);
            i++;
            index++;
            if (index == 48)
                index = 0;
        }else{
            if ((alt_nticks() - time_start) > alt_ticks_per_second())  // 1 second timeout
                bTimeout = TRUE;
        }
    ...
    if (bTimeout){
        AUDIO_DEBUG(("[AUDIO] play1k, timeout\r\n"));
    }else{
        bSuccess = TRUE;
    }
